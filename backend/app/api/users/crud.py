...
async def get_user_by_email(db: AsyncSession, email: str) -> Optional[users_models.User]:
...
async def get_user_by_id(db: AsyncSession, user_id: int) -> Optional[users_models.User]:
    result = await db.execute(select(users_models.User).where(users_models.User.id == user_id))
    return result.scalar_one_or_none()
